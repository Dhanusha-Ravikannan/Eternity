...
//         const response = await axios.get(`${BACKEND_SERVER_URL}/api/transactions/${selectedCustomerId}` );
...
//         //  Calculate totals
//         const totalPurity = transactions.reduce(
//           (sum, item) => sum + (parseFloat(item.purity) || 0),    0       );
//         const totalUsedPurity = transactions.reduce(
//           (sum, item) => sum + (parseFloat(item.usedPurity) || 0),       0       );
  
//         //  Get opening balance from customer data (same for all transactions)
//         const openingBalance = parseFloat(transactions[0]?.customer?.openingBalance || 0);
  
//         //  Final balance formula
//         const balance = (totalPurity - totalUsedPurity) + openingBalance;
  
//         console.log(" Computed Balance:", balance);
  
//         // setCustomerBalance(balance > 0 ? balance : 0);
...
// const totalRowPurity = parseFloat(totalPure) + parseFloat(customerBalance || 0);
// const pureBalanceValue = totalRowPurity - parseFloat(totalReceivedPurity || 0);
...
// {customerBalance < 0 ? (
//   <tr style={{ color: "green", fontWeight: "bold" }}>
//     <td colSpan={5}>Customer Excess Balance (Owner must give to Customer)</td>
//     <td>{(customerBalance).toFixed(3)}</td>
//     <td>{((customerBalance) * goldRate).toFixed(2)}</td>
//     <td></td>
//   </tr>
// ) : (
//   <tr style={{ color: "red", fontWeight: "bold" }}>
//     <td colSpan={5}>Customer Balance (Customer must give to Owner)</td>
//     <td>{(customerBalance).toFixed(3)}</td>
//     <td>{((customerBalance) * goldRate).toFixed(2)}</td>
//     <td></td>
//   </tr>
...
//     {(parseFloat(totalPure) + parseFloat(customerBalance || 0)).toFixed(3)}
//   </td> 
...
//   style={{
//     color:
//       totalAmount - customerBalance * goldRate >= 0 ? "red" : "green",
//     fontWeight: "bold",
//   }}
// >

//   {(parseFloat(totalAmount) + parseFloat(customerBalance || 0) * parseFloat(goldRate || 0)).toFixed(2)}<br/>
...
cashBalance = parseFloat(cashBalance.toFixed(2));
  ...
  //       amount: item.amount,
  //       itemId: {
  //         id: selectedProductId || item.itemId?.id || null,
  //         name: selectedProduct || item.itemId?.name || "",
  //       },
  //       touchValue: item.touchValue || "", // ✅ Add manual touch value
  //     }));
  
  //     // ✅ Format receivedItems to include touchValue
  //     const formattedReceivedItems = receivedRows.map((row) => ({
  //       id: row.id,
  //       date: row.date,
  //       type: row.type,
  ...
  //       touchValue: row.touchValue || "", // ✅ Add manual touch value
  //       purityWeight: row.purityWeight || 0,
  //       amount: row.amount || "",
  //       hallmarkCharge: row.hallmarkCharge || "",
  //     }));
  
  //     // ✅ Now send clean, structured data
  //     const billData = {
  //       customerId: selectedCustomerId,
  //       date: currentDate,
  //       time: time,
  ...
  //       prevHallmark: prevHallmark,
  //       hallmarkBalance: hallmarkBalance,
  //       billItems: formattedBillItems, // ✅ updated
  //       receivedItems: formattedReceivedItems, // ✅ updated
  //       excessPure: Math.abs(pureBalance),
  //     };
